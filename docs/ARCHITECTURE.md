...
| Read-only EDID Viewer GUI | `tools/EdidViewer/EdidViewer.cpp`, `tools/EdidViewer/EdidViewer.vcxproj` | Select a binary EDID file and display identity, aggregate timing health, detailed timings, and advertised CTA modes | Win32 presentation layer over Portable Core | Separate from the platform-independent core; file input only, with no monitor discovery, Registry, serialization, or apply path. |
