...
| Read-only timing candidate generator | `core/include/TimingCandidateGenerator.h`, `core/src/TimingCandidateGenerator.cpp` | Generate bounded fixed-total refresh steps from 60 Hz to the mathematical range estimate, calculate required horizontal rate/pixel clock, and classify each as advertised or experimental | Timing snapshot and range capability estimate | Candidate metadata only. It does not synthesize new porch/sync values, validate GPU/link transport, serialize EDID, or apply a mode. |
...
| Base advertised timings | `core/include/BaseAdvertisedTimings.h`, `core/src/BaseAdvertisedTimings.cpp` | Decode the Established Timing bitmap and eight Standard Timing pairs in the base EDID | Base EDID parser | These compact records provide resolution, refresh, and scan mode only; pixel clock and blanking geometry are intentionally not inferred. |
| GTF timing calculator | `core/include/GtfTimingCalculator.h`, `core/src/GtfTimingCalculator.cpp` | Reproduce the legacy CRU integer GTF calculation as a standard C++ read-only transformation into `TimingSnapshot` | Explicit resolution, refresh, and scan mode | A calculated timing is an estimate, not measured monitor capability and not authorization to apply a mode. |
| Display mode inventory | `core/include/DisplayModeInventory.h`, `core/src/DisplayModeInventory.cpp` | Merge and deduplicate base Established/Standard timings, resolved DTDs, and CTA VIC modes; preserve source/native metadata; summarize advertised minimum/maximum refresh per resolution and scan mode | Display capabilities snapshot | Advertised-mode inventory only; ranges are not overclocking limits or proof of link/GPU support. |
