...
Timing candidate coverage generates 5 Hz steps from 60 Hz through an exact non-step ceiling, verifies required pixel clock/horizontal rate, advertised-to-experimental classification at the EDID boundary, 18-byte DTD clock representability, bounded-output truncation, and rejection of a zero step. No candidate is serialized or applied.
